...
# Importar etl_functions para coverage funcionar
...
class TestDownloadKaggleDataset:
    ...
    def test_download_success(
        ...
        with patch.dict('sys.modules', {'kaggle': MagicMock()}):
            ...
            # Setup mocks - CORRIGIDO: simular que ZIP existe antes de ser extraído
            def exists_side_effect(path):
                ...
                if 'brazilian-ecommerce.zip' in path:
                    return True  # ← CORREÇÃO: ZIP deve existir
                return False
            ...
            # Primeiro listdir retorna ZIP, depois retorna CSVs
            mock_listdir.side_effect = [
                [],  # Primeira chamada (para limpar)
                ['file1.csv', 'file2.csv', 'file3.csv']  # Segunda chamada (após extração)
            ]
...
class TestDAGStructure:
    ...
    def test_dag_default_args_present(self):
        ...
        # Aceitar aspas duplas OU simples
    ...
    def test_dag_dependencies_defined(self):
        ...
        assert '>>' in content or 'set_upstream' in content or 'set_downstream' in content
...
class TestDataValidation:
    ...
    def test_sample_dataframe_values(self, sample_dataframe):
        ...
        assert sample_dataframe['order_status'].isin(
...
class TestIntegration:
    ...
    def test_dag_workflow_sequence(self):
        ...
        assert all(pos != -1 for pos in [download_pos, load_pos, validate_pos, dbt_run_pos, dbt_test_pos])
        
        # Verificar ordem de definição (não necessariamente ordem de execução, mas é um indicador)
